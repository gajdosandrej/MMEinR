mixed <- function(y, X, Z, dim, s20, method, iterlim, steptol) {
...
# ======================================================================
           ...
           H<-matrix()
           q<-vector();
           return(s2,b,u,Is2,C,H,q,loglik,loops)
                }
...
# ======================================================================
        ...
        crit=0 #pre MINQE(I) a MINQE(U,I) zbehne while iba raz
        }
        #stop()
        if(loops==max_iter) {
...
# ======================================================================
        ...
        for(i in 1:r) {
                ...
                for(j in 1:r) {
                        ...
                        tr<-matrix.trace(as.matrix(W[ilow:iupp,jlow:jupp]%*%W[jlow:jupp,ilow:iupp]))
                        trsum<-trsum+tr
                        Is2[i,j]<-(1*(i==j)*(1-2*trii)+tr)/(s2[i]*s2[j])
                }
...
# EOF MIXED.M
# ======================================================================
        result<-list(s2,b,u,Is2,C,H,q,loglik,loops)
        return(result)
}
